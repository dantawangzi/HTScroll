...
public class SimpleParallelSpaceModel implements ParallelSpaceModel {
    ...
    /** Contains the mean and standard deviation of each record*/
    protected List<Float> recordMax;
    protected List<Float> recordMean;
    protected List<Float> recordSD;
    protected List<Float> recordEntropy;
    protected List<Integer> newAxisSequence;
...
//        }
    ...
    /**
     * Returns the maximum value for the given record.
     *
     * @return Maximum value of all records for the given record.
     */
    public float getRecordMaxValue(int row){
        if(recordMax.size() != 0){
            return recordMax.get(row);
        }
        else{
            return -1;
        }
    ...
    public void setRecordMax(List<Float> rMax){
        this.recordMax = rMax;
    }
    /**
     * Returns the mean value for the given record
     */
    public float getRecordMean(int row){
        float meanRValue = -1;
        if(this.recordMean.size() != 0){
            meanRValue = this.recordMean.get(row);
        }
//        for(int i=0; i<numDimensions; i++){
//                meanRValue += ((float[])values.elementAt(row))[i];
//        }
//        meanRValue = meanRValue/(float)numDimensions;
        return meanRValue;
    }
    /**
     * Set the mean value calculated when read in
     */
    public void setRecordMean(List<Float> mean){
        this.recordMean = mean;
    }
    /**
     * Returns the standard deviation value for the given record
     */
    public float getRecordSD(int row){
        float sdValue = 0;
        if(this.recordSD.size()!=0){
            sdValue = recordSD.get(row);
        }
        
        return sdValue;
    }
    public void setRecordSD(List<Float> sd){
        recordSD = sd;
    }

    public void setRecordEntropy(List<Float> en){
        recordEntropy = en;
    }
    public float getRecordEntropy(int row){
        float entropyValue = 0;
        if(this.recordEntropy.size() != 0){
            entropyValue = recordEntropy.get(row);
        }
        return entropyValue;
    }
    /**
    ...
    public void setNewAxisSequence(List<Integer> sequence) {
        if(newAxisSequence == null){
            newAxisSequence = sequence;
        }
    }
    public List<Integer> getNewAxisSequence(){
        return newAxisSequence;
    }
